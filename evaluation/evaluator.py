...
#这里是用来计算fps的
    ...
    def evaluate_video(self, model, video_name, video_parts, output_path):
        for vos_data in video_parts:
            ...
            files = vos_data['files']  #这里的files = ['{:05d}.png'.format(i) for i in frame_ids], 用于组成图片名的一个列表
    ...
    def evaluate(self, model, output_path):
        ...
        for video_name, video_parts in self.dataset.get_videos():# 这里的get_videos()是在davis.py里实现的
            #video_parts 是一个多元组{'imgs': imgs, 'flows': flows, 'masks': masks, 'files': files}

...
# 这个是返回连续的几帧
class TestFBMS2(torch.utils.data.Dataset):
    ...
    def init_data(self):
        img_root_dir = os.path.join(self.root, 'JPEGImages')
        if not os.path.exists(img_root_dir):
            raise FileNotFoundError(f"JPEGImages 目录不存在: {img_root_dir}")
        ...
        for center_frame_idx_in_video in range(total_frames):
            ...
            for frame_id in selected_frame_indices:
                try:
                    ...
                    mask = Image.open(mask_paths[frame_id]).convert('L')
                except IndexError:
            ...
            current_center_mask = self.to_tensor(masks_current_sequence[center_position_in_sequence])
            ...
            stacked_ref_masks_for_this_center = torch.stack([self.to_tensor(mask) for mask in masks_current_sequence])

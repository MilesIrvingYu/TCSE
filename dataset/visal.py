...
#这个是直接复制num_frames份
class TestVISAL(torch.utils.data.Dataset):
    ...
    def init_data(self):
        ...
        for video_name in self.video_list:
            ...
            # VISAL前面的视频都是bmp格式，后两个视频是png格式
            img_paths = sorted(glob(os.path.join(img_dir, '*.bmp')) +
                          glob(os.path.join(img_dir, '*.png')))
            flow_paths = sorted(glob(os.path.join(img_dir, '*.bmp')) +
                          glob(os.path.join(flow_dir, '*.png')))
